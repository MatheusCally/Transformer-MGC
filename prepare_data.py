...
from audio_processing import random_crop, random_mask,rename_tracks
...
def get_id_from_path(path):
    ...
    retorno = base_name.replace(".mp3", "").replace(".npy", "")
    return retorno
...
def left_zeros_file(value):
    return '000' if value >= 10 else '0000';

if __name__ == "__main__":
    in_path = "Data/tracks.csv"
    genres_path = "Data/genres.csv"

    out_path = "Data/tracks_genre.json"
    mapping_path = "Data/mappingGtzan.json"

    # df = load("Data/features_30_sec.csv")
    ...
    for i in genre_list:
        for j in range(0,100):
            if i == 'jazz' and j == 54:
                ...
                data[str(genre_list.index(i)) + left_zeros_file(j) + str(j)] = i
                

    json.dump(data, open(out_path, "w"), indent=4)
    # base_path = "./Data/tracks/"
    # for i in genre_list:
    #     for j in range(0,100):
    #         os.rename(base_path + i + left_zeros_file(j) + str(j) + '.wav', base_path + str(genre_list.index(i)) + left_zeros_file(j) + str(j) + '.wav')
